...
class DatacenterAuthMessageService : public MessageService, public std::enable_shared_from_this<DatacenterAuthMessageService> {
    public:
        ...
        void
        protoDidReceiveMessage(const std::shared_ptr<Proto> &proto, std::shared_ptr<IncomingMessage> message) override;

        void
        protoTransactionsMayHaveFailed(const std::shared_ptr<Proto> &proto, std::vector<int> transactionIds) override;

        void protoMessageDeliveryFailed(const std::shared_ptr<Proto> &proto, int64_t messageId) override;

        void
        protoMessagesDeliveryConfirmed(const std::shared_ptr<Proto> &proto, std::vector<int64_t> messages) override;
    ...
    private:
        ...
        int64_t currentStateMessageId;
        int32_t currentStateMessageSeqNo;
        int currentStateTransactionId;
